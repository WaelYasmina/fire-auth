...
checkBtn.addEventListener('click', () => {
  if (fileBrowse != null) {
    ...
    let storageRef = storage.ref('weightsfile/' + fileBrowse.name) //스토리지
    storageRef.put(fileBrowse).then(() => {
      let fileLink = storage.ref(`weightsfile/${fileBrowse.name}`)
      urlDownload = fileLink
        ...
        .then(() => {
          let docRef = firestore.collection('weightsfile') //데이터베이스 저장소
        ...
        .then((data) => {
          data.forEach((element) => {
            element.ref.delete().then(() => {
              let storageRef = storage.ref('images/' + idFile) //스토리지
